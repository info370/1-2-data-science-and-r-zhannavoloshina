...
length.na <- length(which(is.na(data$at_scene_time_date)))
prop <- length.na / total_reports

length.na.clearence <- length(which(is.na(data$event_clearance_ts)))
prop.clearence <- length.na.clearence / total_reports
```
...
```{r}
hoursData = data.frame(data[,table((event_clearance_hr))]); 
View(hoursData)
